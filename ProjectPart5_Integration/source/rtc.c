...
void Rtcc_periodicTask( Rtcc_Clock *rtcc ) {
    ...
    Sched_Scheduler_t *timerScheduler;
    Sched_Timer_t *currentTimer;
    uint8_t timerID = Sched_registerTimer(timerScheduler, 1000u, NULL);
    ...
    while (1) {
        ...
        if(generalTickElapsedTime > 1000) {
            generalTickElapsedTime = 0;
            tickCounterFlag = TRUE;
        }

        if( tickCounterFlag == TRUE ) {
